...
<head>
    ...
    <style>
        ...
        .brand-panel { background: radial-gradient(circle at top left, rgba(227,179,65,.14), transparent 32%), radial-gradient(circle at bottom right, rgba(200,16,46,.22), transparent 38%), linear-gradient(135deg, #06060a 0%, #9D5268 50%, #E97C7F 130%); }
    </style>
</head>
...
<body class="min-h-screen bg-[#F5F5F3] flex items-center justify-center p-4">
    ...
    <div class="w-full max-w-md">
        ...
        {{-- Header card --}}
        <div class="brand-panel relative overflow-hidden rounded-[20px] text-white px-6 py-6 shadow-[0_15px_45px_rgba(233,124,127,0.35)] mb-4">
            <div class="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-[#E3B341] via-[#E3B341] to-[#E3B341]/10"></div>
            <div class="pointer-events-none absolute -top-10 -right-10 w-40 h-40 rounded-full bg-[#E3B341]/10 blur-3xl"></div>
            <div class="relative flex items-center gap-3 mb-1">
                <div class="w-11 h-11 rounded-xl overflow-hidden ring-2 ring-[#E3B341] shrink-0">
                    <img src="{{ asset('images/AI-RCG.png') }}" alt="RCG" class="w-full h-full object-cover">
                </div>
                <div>
                    <h1 class="text-base font-black">Select Company</h1>
                    <p class="text-[11px] text-[#E3B341]/80 font-semibold">Choose which company dashboard to access</p>
                </div>
            </div>
        ...
        <div class="space-y-3">
            @forelse($dashboards as $dashboard)
                ...
                <form method="POST" action="{{ route('dashboard.select') }}">
                    ...
                    <button type="submit" class="w-full text-left bg-white rounded-2xl border border-[#E5E7EB] border-t-[3px] border-t-[#E3B341] shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all group">
                        <div class="p-4 flex items-center gap-4">
                            ...
                            <div class="w-12 h-12 rounded-xl bg-slate-50 flex items-center justify-center shrink-0 overflow-hidden border border-[#E5E7EB]">
                                ...
                                @else
                                    <span class="text-sm font-black text-[#E97C7F]">{{ strtoupper(substr($dashboard['company_code'],0,2)) }}</span>
                                @endif
                            </div>
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="mt-2 flex flex-wrap gap-1.5">
                                    ...
                                    @if($dashboard['is_default'] ?? false)
                                        <span class="px-2 py-0.5 rounded-full bg-[#E3B341]/10 text-[#B8860B] text-[10px] font-bold border border-[#E3B341]/30">Default</span>
                                    @endif
                                </div>
                            </div>
        ...
        <form method="POST" action="{{ route('logout') }}" class="mt-5 text-center">
            @csrf
            <button class="text-xs text-slate-400 hover:text-[#E97C7F] transition font-semibold">← Back to login</button>
        </form>
